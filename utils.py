...
from sklearn.metrics import adjusted_rand_score, normalized_mutual_info_score, adjusted_rand_score
...
def euclidean_distance(data1, data2, device=device):
    # transfer to device
...
def cosine_sim(data1, data2, device=device):
    """
...
def cosine_distance(data1, data2, device=device):
    """\
...
def kl_div_matrix(matrix1, matrix2, temperature=1.0, device=device):
...
def symmetric_kl_divergence_sample(x_1, x_2, t=1.0):
    ...
    skl_dev = (skl_dev + torch.sum(
        p_x_2_sample * (torch.log(p_x_2_sample) - torch.log(p_x_1_sample))) / p_x_2_sample.size(0)) / 2
