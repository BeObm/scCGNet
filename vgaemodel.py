...
def normalize_log_counts(counts, target_sum=1e4):
    """
    Normalize raw gene counts and apply log1p transformation.
    ...
    log_counts : np.ndarray
        Normalized/log-transformed matrix [cells, genes].
    """

    X = np.asarray(counts, dtype=np.float32)

    # Library size for each cell/sample
    ...
    X_norm = X / library_size * target_sum
